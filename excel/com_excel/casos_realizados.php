...
$sql = "SELECT ca.id_caso,ca.fecha_creacion,us.nombres,us.apellidos,tip.tipo_proceso,esp.estado_proceso,ca.fecha_modificacion,us.correo 
                FROM caso ca 
...
$objPHPExcel->getActiveSheet()->getStyle('A1:G3')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
$objPHPExcel->getActiveSheet()->getStyle('A1:G3')->getFill()->getStartColor()->setARGB('ffffff'); //357320
$objPHPExcel->getActiveSheet()->mergeCells('A1:G3'); //Unir celdas

$objPHPExcel->getActiveSheet()->getStyle('A4:G5')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
$objPHPExcel->getActiveSheet()->getStyle('A4:G5')->getFill()->getStartColor()->setARGB('0000cc'); //357320
$objPHPExcel->getActiveSheet()->getStyle('A4:G5')->getFont()->getColor()->setARGB(PHPExcel_Style_Color::COLOR_WHITE);
...
$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('C5', 'Correo institucional')
        ->setCellValue('D5', 'Tipo solicitud')
        ->setCellValue('E5', 'Estado solicitud')
        ->setCellValue('F5', 'Fecha creacion')
        ->setCellValue('G5', 'Fecha ultima modificacion');

$objPHPExcel->getActiveSheet()->getStyle('A5:G5')->getFont()->setBold(true);
$objPHPExcel->getActiveSheet()->setAutoFilter('A5:G5');
$i = 6;
//while ($value = $resul){ 
...
foreach ($resul as $key => $value) {
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C' . $i, utf8_encode($value['correo']));
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D' . $i, utf8_encode($value['tipo_proceso']));
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E' . $i, utf8_encode($value['estado_proceso']));
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F' . $i, $value['fecha_creacion']);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G' . $i, $value['fecha_modificacion']);

    if ($i % 2 == 0) {
        $objPHPExcel->getActiveSheet()->getStyle('A' . $i . ':G' . $i)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
        $objPHPExcel->getActiveSheet()->getStyle('A' . $i . ':G' . $i)->getFill()->getStartColor()->setARGB('ffffff'); //357320
    } else {
        $objPHPExcel->getActiveSheet()->getStyle('A' . $i . ':G' . $i)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
        $objPHPExcel->getActiveSheet()->getStyle('A' . $i . ':G' . $i)->getFill()->getStartColor()->setARGB('ffffff'); //357320
    }
...
$objPHPExcel->getActiveSheet()->setTitle('Registro_de_atencion_de_casos');
